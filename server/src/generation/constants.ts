...
// A fresh RNG is created on every call, seeded from this base value plus the
// months-since-founding of the month being simulated (see events.ts), not
// the constant alone. Each tick targets a new month (tracked via
// next_tick_month regardless of whether the prior month produced any
// events — see schema.ts), so this seed is never reused, and a repeat click
// never replays the same draw (hire probability, team, role, name) forever.
